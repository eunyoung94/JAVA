...
/*
 BoardApp가 모든페이지들을 보유하고 있는 최상위 컨테이너이므로, 각 페이지들마다 서로 공유할 데이터가 있다면
 BoardApp에 데이터를 두고 처리하면된다. 
 */
...
public class BoardApp extends JFrame{
	...
	//이 모든 페이지들이 사용할 접속정보 객체를 공통으로 선언, 이 커넥션 객체는 프로그램이 가동과 동시에 접속을 얻어다 놓자
	private Connection con;
	//로그인 상태 여부를 보관할 변수 
	
	private boolean hasSession = false;//세션을 보유하고있는지..

	//회원정보를 보관할 자료형
	//이 변수에 데이터가 채워지는 시점은? 로그인 성공할때
	private BoardMember boardMember; //현재는 null 
	
	public BoardApp() {
		this.getConnection(); //프레임을 보여주기 직전에 데이터베이스 접속에 성공해놓자 
		//생성
		p_north = new JPanel();
		bt_board = new JButton(ImageUtil.getIcon(this.getClass(), "res/board.png", 60, 60));
		bt_regist = new JButton(ImageUtil.getIcon(this.getClass(), "res/regist.png", 60, 60));
		bt_login = new JButton(ImageUtil.getIcon(this.getClass(), "res/login.png", 60, 60));
		p_center = new JPanel();
		...
		//스타일 
		bt_board.setPreferredSize(new Dimension(60, 60));
		bt_regist.setPreferredSize(new Dimension(60, 60));
		bt_login.setPreferredSize(new Dimension(60, 60));
		...
		//디폴트로 보여질 페이지와 않보이게 될 페이지에 대한 처리 , 로그인체크 
