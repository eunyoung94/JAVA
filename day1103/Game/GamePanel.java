...
public class GamePanel extends JPanel {
	...
	boolean flag =false; //최초에는 게임이 멈춰 있어야하므로..
	boolean over =false; //game over띄울지 결정하는변수
	int Score=0; //점수
	...
	public void fire() {
		...
		Bullet bullet = new Bullet(this, img, hero.x + hero.width, hero.y + (hero.height / 2), 30, 30, 10, 0);
...
//			System.out.println(n);
	...
	public void createHP() {
		...
		for(int i=0;i<4;i++) {
			HP hp= new HP(img,50+(33*i),85,30,30,0,0);
			hpList.add(hp);
		}
	}
	...
	public void printData(Graphics2D g2) {
		...
		if(over)showGameOver(g2);//게임종료메세지 띄우기 
	}
	...
	public void render(Graphics2D g2) {
		...
		for (int i = 0; i < enemyList.size(); i++) {
			Enemy enemy = enemyList.get(i);
			enemy.render(g2);// 다수의 총알에 대한 .tick()
		}
		for (int i = 0; i < blockList.size(); i++) {
			Block block = blockList.get(i);
			block.render(g2);// 다수의 총알에 대한 .tick()
		}
		for(int i=0;i<hpList.size();i++) {
			HP hp = hpList.get(i);
			hp.render(g2);
