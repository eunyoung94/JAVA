/*각종 컴포넌트를 올려놓기 전에, 어던방식으로 올려놓을지를 결정하는 것을 배치라하며 AWT에서 지원하는 배치는 다음과 같이 5가지 유형이 있다. 
1)FlowLayout:행방향으로 늘어서는 배치방식, 만일 공간이 부족하다면 다음행으로 내려간다. 컨테이너의 크기에 따라 흐르듯 배치가 되므로 flow라는 뜻이부여되었다.
2)BorderLayout: 북,동,서,남,중앙 이라는 방위를 갖는 배치방법
3)GridLayout: 격자 즉 바둑판 모양의 배치방법
4)CardLayout: 카드처럼 하나의 카드가 보일때는 다른 카드가 안보이는 방식의 배치
GridBagLayout : GridLayout을 픽셀단위로 보다 상세하게 제어할 수 있는 배치방식 
사실상 1,2,3 번만으로도 충분히 개발가능함...
*/
package gui;
